...
"tipus" per distingir-los, indicant "M" per màtemàtiques o "P" per
portuguès.
...
Al següent pas convertirem les dades de tipus caràcter a factor i en
mostrarem un resum numèric de totes elles
...
i en tornem a mostrar el gràfic sense aquests.
...
Agunes correlacions interessants relacionades amb la nota (*grade*) que
podrien plantejar un estudi són:
...
*final grade* (G3) en funció de diferents variables categòriques:
...
for(i in 1:ncol(alumnes)) {
  if (is.factor(alumnes[,i])){
    boxplot(alumnes$grade_mean ~ alumnes[,i], main = colnames(alumnes)[i], ylab = 'Grade', xlab = '')
  }
  if (i == 1){
...
-   G \~ sex
-   G \~ address
-   G \~ assignatura \<- falta definir i omplir variable
-   G \~ nursery
-   G \~ higher
-   G \~ internet
-   
-   G \~ Mjob(health) or Fjob(teacher)

Per acabar el plantejament dels anàlisis, creo una variable dicotòmica
anomenada \*approved\* que, en funció de G3 (o la mitja entre les G),
identifiqui si un estudiant ha aprovat o no.

-   Aprovat (approved = 1): G => 10
-   Suspens (approved = 0): G \< 10

```{r 9-8}
alumnes$approved <- ifelse(alumnes$grade_mean>=10, 1, 0)

alumnes$approved <- factor(alumnes$approved)

tail(alumnes$approved, 10)
tail(alumnes$G3, 10)
...
```{r 0-9}
with(alumnes, Barplot(Fjob, by=approved, style="divided", 
  legend.pos="above", ylab="percent", scale = "percent",
  label.bars=FALSE))

with(alumnes, Barplot(factor(Walc), by=approved, style="divided", 
  legend.pos="above", ylab="percent", scale = "percent",
  label.bars=FALSE))
```
...
```{r comprovacions_normalitat, fig.width=6, fig.height=8}
...
```{r transformació_normal, fig.width=4, fig.height=4}
...
```{r 0-7a, fig.width=10, fig.height=6}
...
target_variables <- c("sex", "address", "school", "nursery", "internet", "famsize")
...
for (each_variable in target_variables){
  ...
  if(each_variable=="famsize"){
    pval <- t.test(subsets[[1]]$grade_mean,subsets[[2]]$grade_mean, 
                   alternative="two.sided", 
                   var.equal=TRUE)$p.value
  }else if (mean(subsets[[1]]$grade_mean) > mean(subsets[[2]]$grade_mean)){
    pval <- t.test(subsets[[1]]$grade_mean,subsets[[2]]$grade_mean, 
                   alternative="greater", 
                   var.equal=TRUE)$p.value
  }
...
6.  Com abans, el valor p obtingut porta a rebutjar la hipòtesi nul·la d'igualtat
    de mitjanes de notes (*Grades)* entre alumnes amb accés família nombrosa o
    no. Amb un nivell de confiança del 95%, **es pot afirmar
    estadísticament que hi ha diferències significatives entre els *Grades*
    d'alumnes amb família nombrosa i alumnes que no tenen família nombrosa**.
...
no-paramètriques porta a les mateixes conclusions que els p-value
resultants de les proves paramètriques. Per tant, es pot concloure que
la assumpció (opció A detallada en el subapartat anterior) de normalitat
poblacional -i la conseqüent desviació respecte a la normal- no genera
cap impacte significatiu en els resultats dels anàlisis plantejats.

### 4.3.3. Creació d'un model generalitzat

```{r model_generalitzat}

mod.gen <- lm(G3 ~ goout + failures + age , data = alumnes)

#GLMa <- glm(formula=approved~failures+factor(goout),family=binomial(link=logit), data = alumnes)
#summary(GLMa)
```
